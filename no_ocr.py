...
def custom_ocr(video_path, output_video_path, weights_path, digit_model_path):
    ...
    def detect_digits_with_adjustments(digit_model, number_plate_region):
        attempts = [
            ...
            lambda img: resize_region(img, 1.2),  # Zoom in
            lambda img: resize_region(img, 0.9),  # Zoom out
            lambda img: adjust_brightness_contrast(img, 1.5, 50),  # Increase contrast
            lambda img: cv2.GaussianBlur(img, (5, 5), 0)  # Blur noise reduction
        ]
    ...
    while True:
        ...
        for result in results[0].boxes:
            ...
            if label == 'Number Plate':
                ...
                cv2.putText(frame, 'Number Plate', (int(x1), int(y1 - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
                ...
                number_plate_digits.extend(sorted_digits)

        if len(number_plate_digits) == 5:
            stored_digits = "".join([digit[4] for digit in number_plate_digits])

        if stored_digits:
            cv2.putText(frame, f"Detected Digits: {stored_digits}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
...
def process_video_and_ocr(video_path, selected_labels):
    ...
    weights_dictionary = {
        ...
        "Digit Model" : os.path.join("weights","digit_detection.pt")
    }
    ...
    if "Illegal Parking" in selected_labels:
        ...
        elif "Number Plate" in selected_labels:
            weights_path = weights_dictionary["Other"]
            digit_model_path = weights_dictionary["Digit Model"]      
            
            custom_ocr(video_path, output_video_path, weights_path, digit_model_path)
        
        elif "Garbage" in selected_labels or "pedestrian" in selected_labels or "street light" in selected_labels or "Construction Material" in selected_labels:  
            weights_path = weights_dictionary["Other"]
